...
int wb_http_parse_request_line(wb_http_request_t *request) {
    ...
    for(pi = request->pos; pi < request->last; ++pi) {
        ...
        case sw_start:
            ...
            if((ch < 'A' || ch > 'Z') && ch != '_') {
                return WB_HTTP_PARSE_INVALID_METHOD;
            }
            state = sw_method;
            break;
        ...
        case sw_method:
            if(ch == ' ') {
                ...
                switch(p - m) {
                case 3:
                    if(wb_str3_cmp(m, 'G', 'E', 'T', ' ')) {
                        request->method = WB_HTTP_GET;
                        break;
                    }
                    break;
                case 4:
                    if(wb_str30cmp(m, 'P', 'O', 'S', 'T')) {
                        request->method = WB_HTTP_POST;
                        break;
                    }
                    if(wb_str4cmp(m, 'H', 'E', 'A', 'D')) {
                        request->method = WB_HTTP_HEAD;
                        break;
                    }
                    break;
                default:
                    request->method = WB_HTTP_UNKNOWN;
                    break;
                }
        ...
        case sw_spaces_before_uri:
            ...
            case ' ':
                break;
            default:
                return WB_HTTP_PARSE_INVALID_REQUEST;
            }
            break;
    ...
    for(pi = request->pos; pi < request->last; ++pi) {
        ...
        case sw_spaces_before_colon:
            if(ch == ' ') {
                ...
                return WB_HTTP_PARSE_INVALID_HEADER;
            }
        ...
        case sw_cr:
            if(ch == LF) {
                state = sw_crlf;
                hd = (wb_http_header_t *)malloc(sizeof(wb_http_header_t));
